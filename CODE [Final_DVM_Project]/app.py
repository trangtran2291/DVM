...
def login():
    if request.method == 'POST':
        ...
        if user and bcrypt.checkpw(password, user['mat_khau']):
...
def get_all_products():
    ...
    query = """
        SELECT SanPham.ten_san_pham, SanPham.gia_ban, KichCo.ten_kich_co, 
               MauSac.ten_mau, SanPham.hinh_anh_chinh
        FROM SanPham
        LEFT JOIN BienTheSanPham ON SanPham.id = BienTheSanPham.id_san_pham
        LEFT JOIN KichCo ON BienTheSanPham.id_kich_co = KichCo.id
        LEFT JOIN MauSac ON BienTheSanPham.id_mau_sac = MauSac.id
        LEFT JOIN DanhMucSanPham ON SanPham.id_danh_muc = DanhMucSanPham.id
    """
...
def filter_products():
    ...
    query = """
        SELECT BienTheSanPham.id, SanPham.ten_san_pham, SanPham.gia_ban, KichCo.ten_kich_co, 
               MauSac.ten_mau, DanhMucSanPham.ten_danh_muc, SanPham.hinh_anh_chinh
        FROM BienTheSanPham
